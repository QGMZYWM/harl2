...
class OffPolicyBaseRunner:
    ...
    def __init__(self, args, algo_args, env_args):
        ...
        if not self.algo_args["render"]["use_render"]:
            # Check if Transformer is enabled and adjust share observation space accordingly
            share_obs_space = self.envs.share_observation_space[0]
            
            # If using Transformer, adjust the share observation space to match the context embedding dimension
            if algo_args["model"].get("use_transformer", False):
                # Get the Transformer context dimension
                transformer_d_model = algo_args["model"].get("transformer_d_model", 256)
                
                # Create a modified share observation space with the Transformer context dimension
                from gym import spaces
                share_obs_space = spaces.Box(
                    low=-float('inf'),
                    high=float('inf'),
                    shape=(transformer_d_model,),
                    dtype=np.float32
                )
                
                print(f"🔧 Transformer enabled: adjusted share observation space to {transformer_d_model} dimensions")
